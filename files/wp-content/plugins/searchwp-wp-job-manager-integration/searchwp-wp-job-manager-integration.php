...
Copyright 2014-2020 Jonathan Christopher
...
if ( ! defined( 'SEARCHWP_WPJOBMANAGER_VERSION' ) ) {
	define( 'SEARCHWP_WPJOBMANAGER_VERSION', '1.6.1' );
}
			...
			if ( ! empty( $this->query ) && class_exists( 'SearchWP' ) ) {
				...
				if ( function_exists( 'SWP' ) ) {
					...
					$searchwp_results = new \SWP_Query( array(
						...
						'engine'   => $this->search_engine_name,
						'fields'   => 'ids',
						'nopaging' => true
					) );
				...
				// Sort the results by SearchWP relevance if no ORDERBY is set.
				if ( empty( $query_args['orderby'] ) && ! empty( $query_args['post__in'] ) ) {
