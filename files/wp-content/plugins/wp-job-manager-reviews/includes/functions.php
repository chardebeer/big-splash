...
/**
 * Get Default Star Number.
 * Return the default number of stars used to display. Default is 3;
 *
 * @since 2.2.0
 *
 ...
 * @return int
 */
function wpjmr_get_default_stars( $category = '' ) {
	$max = wpjmr_get_max_stars();
	$default = absint( get_option( 'wpjmr_star_default', 3 ) );
	$default = $default > $max ? $max : $default; // Make sure it do not exceed max star.
	return absint( apply_filters( 'wpjmr_default_star_count', $default, $category ) );
}
	...
	<div class='wpjmr-list-reviews'>
		<?php foreach ( $ratings as $category => $rating ) :
			...
			<div class='stars-rating star-rating'>
				...
				<?php for ( $i = 0; $i < wpjmr_get_max_stars(); $i++ ) : ?><span class="dashicons dashicons-star-<?php echo $i < $rating ? 'filled' : 'empty'; ?>"></span><?php endfor; ?>
			</div>
		<?php endforeach; ?>
	</div>
<?php
...
function wpjmr_send_mail( $args ){
	...
	$email_restriction = (get_option('wpjmr_restrict_emails')==1) ? true : false;
	...
	$args = apply_filters( 'wpjmr_send_mail_args',$email_restriction, $args );
...
function wpjmr_send_mail_args_callback( $bool, $arg1 ) {
	$returnValue = ($bool==false) ? $arg1 : $bool;
	return $returnValue;	
}
add_filter('wpjmr_send_mail_args', 'wpjmr_send_mail_args_callback', 10, 3);
