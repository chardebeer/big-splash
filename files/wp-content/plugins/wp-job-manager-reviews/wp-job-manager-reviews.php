...
class WP_Job_Manager_Reviews {
	...
	public function wpjmr_enqueue_scripts() {
		// General stylesheet.
		wp_enqueue_style( 'wp-job-manager-reviews', plugins_url( 'assets/css/wp-job-manager-reviews.min.css', __FILE__ ), array( 'dashicons' ) );

		// Javascript.
		wp_enqueue_script( 'wp-job-manager-reviews-js', plugins_url( 'assets/js/wp-job-manager-reviews.min.js', __FILE__ ), array( 'jquery' ) );
	}
	...
	public function wpjmr_admin_enqueue_scripts( $hook_suffix ) {
		...
		if ( in_array( $hook_suffix, array( 'comment.php', 'edit-comments.php' ) ) || 'post.php' === $hook_suffix && 'job_listing' === $post_type ) {
			wp_enqueue_style( 'wpjmr-gallery-admin', $this->plugin_url . 'assets/css/wp-job-manager-reviews-gallery-admin.min.css', array() );
		}
	}
